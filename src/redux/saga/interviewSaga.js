...
function* getInterviewId(action) {
  ...
  yield put({
    type: "interviewResult/setInterviewResult",
...
function* createInterview(action) {
  try {
    ...
    yield call(
      axios.post,
      `https://leetun2k2-001-site1.gtempurl.com/api/Interview/${action.payload.interview.applicationId}`,
      action.payload
    );
...
function* getInterviewInfo(action) {
  try {
    const response = yield call(
      axios.get,
      `https://leetun2k2-001-site1.gtempurl.com/api/Interview?id=${action.payload}`
    );
    const response1 = yield call(
      axios.get,
      `https://leetun2k2-001-site1.gtempurl.com/api/Itrsinterview/?id=${response.data.itrsinterviewId}`
    );
    ...
    const room = response2.data.filter(
      (prop) => prop.roomId === response1.data.roomId
    );
    ...
    const shift = response3.data.filter(
      (prop) => prop.shiftId === response1.data.shiftId
    );
    ...
    const interviewer = response4.data.filter(
      (prop) => prop.itrsinterviewId === response1.data.itrsinterviewId
    );
    ...
    const response7 = yield call(
      axios.get,
      `https://leetun2k2-001-site1.gtempurl.com/api/Application`
    );
    const application = response7.data.filter(
      (prop) => prop.applicationId === response.data.applicationId
    );
    const response8 = yield call(
      axios.get,
      `https://leetun2k2-001-site1.gtempurl.com/api/Position/GetPositionById?positionId=${application[0].position.positionId}`
    );
    console.log("appinsaga", application);
    let skilllist = [];
    console.log("reponse8", response8.data);
    const response9 = yield call(
      axios.get,
    ...
    for (let i = 0; i < response8.data.requirements.length; i++) {
      for (let j = 0; j < response9.data.length; j++) {
        if (
          response8.data.requirements[i].skillId === response9.data[j].skillId
        ) {
    ...
    yield put({
      type: "interviewdepartment/setInterviewDepartment",
      payload: department,
    });
    yield put({
      type: "interviewinterviewer/setInterviewInterviewer",
    ...
    yield put({
      type: "interviewposition/setInterviewPosition",
      payload: response8.data,
    });
...
function* getDataForInterview(action) {
  ...
  const responseAppList = yield call(
    axios.get,
    `https://leetun2k2-001-site1.gtempurl.com/api/Application`
  );
  ...
  const responsePositionList = yield call(
    axios.get,
    `https://leetun2k2-001-site1.gtempurl.com/api/Position`
  );
  const findPosition = responsePositionList.data.find(
    (item) => item.positionId == appItem.position.positionId
  );
  const responseDepartmentList = yield call(
    axios.get,
    `https://leetun2k2-001-site1.gtempurl.com/api/Department`
  );
  const findDepartment = responseDepartmentList.data.find(
    (item) => item.departmentId == findPosition.department.departmentId
  );
  console.log("appItem: ", appItem);
  // get upcoming interview
  const responseInterviewList = yield call(
    axios.get,
    `https://leetun2k2-001-site1.gtempurl.com/api/Interview`
  );
  let interviewList = [];
  for (let resInter of responseInterviewList.data) {
    const responseItrsList = yield call(
      axios.get,
      `https://leetun2k2-001-site1.gtempurl.com/api/Itrsinterview?id=${resInter.itrsinterviewId}`
    );
    const responseRoomList = yield call(
      axios.get,
      `https://leetun2k2-001-site1.gtempurl.com/api/Room`
    );
    const findRoom = responseRoomList.data.find(
      (item) => item.roomId == responseItrsList.data.roomId
    );
    const newInter = {
      ...
      interviewdate:
        responseItrsList.data.dateInterview.slice(0, 11) +
        "17" +
        responseItrsList.data.dateInterview.slice(13) +
        ".000Z",
  ...
  const responseDepartInterviewerList = yield call(
    axios.get,
    `https://leetun2k2-001-site1.gtempurl.com/api/Interviewer`
  );
  const filterInterviewerList = responseDepartInterviewerList.data.filter(
    (item) => item.departmentId == findPosition.department.departmentId
  );
  let interviewerList = [];
  ...
  // Get shift
  const responseShiftList = yield call(
    axios.get,
    `https://leetun2k2-001-site1.gtempurl.com/api/Shift`
  );
