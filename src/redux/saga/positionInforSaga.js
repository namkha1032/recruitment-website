...
function* getPositioninfor(action) {
    function convertToTimeZone(originalDateString, timeZoneOffset) {
        ...
        const targetDate = new Date(originalDate.getTime() + timeZoneOffset * 60 * 1000);
    ...
    position[0].startDate = convertToTimeZone(position[0].startDate, targetTimeZoneOffset);
    position[0].endDate = convertToTimeZone(position[0].endDate, targetTimeZoneOffset);
